...
import json
from model.modeling_ubmoe import UBMoeModel
def caculate_dp_one(attention_weight: torch.Tensor, max_n:int=100)->dict:
    dp_result={}
    #print(attention_weight.size())
    if max_n>attention_weight.size()[-1]:
        max_n=attention_weight.size()[-1]
        warnings.warn("555")
...
if __name__ == '__main__':
    ...
    model = UBMoeModel.from_pretrained(model_path)
        ...
        if dp_result is None:
            ...
            for layer_key in dp_one.keys():
                for iter_key in dp_one[layer_key]:
                    if iter_key not in dp_result[layer_key]:
